...
const ProductPage = () => {
    ...
    useEffect(() => {
        ...
        switch (categoryData?.length) {
            case 1:
                d = productDatas && productDatas.filter((i) => i.category[0] === categoryData[0]).sort((a, b) => a.total_sell - b.total_sell)
                setProductsData(d);
                break;
            case 2:
                d = productDatas && productDatas.filter((i) => i.category[0] === categoryData[0] && i.category[1] === categoryData[1]).sort((a, b) => a.total_sell - b.total_sell)
                setProductsData(d);
                break;
            case 3:
                d = productDatas && productDatas.filter((i) => i.category[0] === categoryData[0] && i.category[1] === categoryData[1] && i.category[2] === categoryData[2]).sort((a, b) => a.total_sell - b.total_sell)
                setProductsData(d);
                break;
            default:
                d = productDatas && productDatas.sort((a, b) => a.total_sell - b.total_sell)
                setProductsData(d);
                break;
        }
        window.scrollTo(0, 0);
