...
interface ProductDetailCardProps { productData: ProductData | null; setOpen: (i: boolean, productData: ProductData | null) => void; }

const ProductDetailCard = ({ setOpen, productData }: ProductDetailCardProps) => {
        ...
        <div className='bg-[#ffffff]'>
            {productData ? (
                <div className='fixed w-full h-screen top-0 left-0 bg-[#00000030] z-40 flex items-center justify-center'>
                    <div className='w-[95%] h-[75vh] md:w-[70%] md:h-[75vh] overflow-y-scroll bg-[#ffffff] rounded-md  relative p-3'>
                        <RxCross1 size={25} onClick={() => setOpen(false, null)} className='absolute right-3 top-3 z-50' />
                        <div className='w-full md:flex block'>
                            ...
                            <div className='w-full md:w-[50%] p-5'>
                                <h1 className='font-extrabold text-[20px] mb-1'>{productData.name}</h1>
                                ...
                                <div className='flex font-bold text-[20px] mb-5 space-x-2'>
                                    ...
                                    <h1 className='text-[20px] '>{productData.total_sell} 판매</h1>
                                </div>
                                ...
                                <div className='flex tems-center justify-between flex-col space-y-1 w-full text-center'>
                                    ...
                                    <div className=' flex w-full justify-center bg-white p-3 rounded-md cursor-pointer hover:opacity-50 border-[1px] border-black' onClick={() => setClick(!click)}>
                                        {click
                                            ? (<AiFillHeart size={25} color={click ? "red" : "black"} title='Remove from wishlist' className='mr-3 cursor-pointer' />)
                                            : (<AiOutlineHeart size={25} color={click ? "red" : "black"} title='Add to wishlist' className='mr-3 cursor-pointer' />)
                                        }
                                        관심목록
                                    </div>
                                    ...
                                    <Link to={`/product/${productData.id}`} className=' flex w-full justify-center bg-white p-3 rounded-md cursor-pointer hover:opacity-50 border-[1px] border-black'> <CiSearch size={25} className='mr-3' /> 상품보기</Link>
                                </div>
                            </div>
                        </div>
                    </div>
