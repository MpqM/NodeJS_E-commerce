...
import PromotionProductCard from './PromotionCard';
...
const Promotion = () => {
    ...
    return (
        <div>
            <h1 className='w-11/12 mx-auto border-[1px] border-black font-medium rounded-md text-[20px] mb-3 text-center'>이벤트</h1>
            ...
            <div className='w-11/12 mx-auto mb-3 relative group rounded-md border-[1px] border-black'>
                ...
                <div className="absolute bottom-2 right-0 left-0 flex items-center justify-center gap-2">
                    {promotionData.map((i, slideIndex) => (< div key={i.id} className={`transition-all w-3 h-3 bg-black rounded-full ${currentIndex === slideIndex ? "p-1" : "bg-opacity-25"}`} />))}
                </div>
            </div>
        </div>
    )
