    ...
    return (
        <>
            <div className='w-11/12 mx-auto mb-3'>
                {/* 상위 헤더 */}
                <div className='h-[50px] my-[20px] flex md:flex flex-col md:flex-row items-center justify-between'>
                    {/* 제목, 홈페이지 링크 */}
                    <div><Link to="/" className='flex md:block text-sm md:text-3xl font-extrabold mb-1'>E-COMMERCE</Link></div>

                    {/* 상품 검색 */}
                    <div className='w-full md:w-[50%] relative'>
                        <input
                            type="text" placeholder="검색" value={searchTerm} onChange={handleSearchChange}
                            className='flex w-full md:w-full h-[40px] p-1 border-[1px] border-black focus:outline-none rounded-md'
                        />
                                    ...
                                    return (
                                        <Link key={i.id} to={`/product/${i.id}`}>
                                            <div className="w-full flex items-start p-1 hover:opacity-50">
                                                ...
                                                <p className='text-sm md:text-md ml-1'>{i.name}</p>
                                            </div>
                                        </Link>
                                    );
                                })}
                ...
                {/* 하위 헤더 */}
                <div className={`${active === true ? "shadow fixed top-0 left-0 z-10 rounded-none border-none" : null} transition hidden md:flex items-center justify-between w-full bg-white border-[1px] border-black h-[50px] rounded-md`}>
                    {/* 카테고리 드롭다운 */}
                ...
                {/* 하위헤더 모바일용 */}
                <div className='flex md:hidden mx-auto mb-3 '>
                    <button
